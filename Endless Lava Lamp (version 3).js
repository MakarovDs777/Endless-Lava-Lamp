var createScene = function () {
    ...
    scene.registerBeforeRender(function () {
        ...
        for (var i = 0; i < lavaBalls.length; i++) {
            ...
            if (targetPosition.x < 0 || targetPosition.x > 100) {
                lavaBall.vx *= -1;
            }
            if (targetPosition.y < 0 || targetPosition.y > 10) {
                lavaBall.vy *= -1;
            }
            if (targetPosition.z < 0 || targetPosition.z > 100) {
                lavaBall.vz *= -1;
            }

            // Обновляем целевую позицию после отражения
            ...
            // Линейная интерполяция между текущей и целевой позициями
            lavaBall.sphere.position = BABYLON.Vector3.Lerp(lavaBall.sphere.position, targetPosition, 0.1);
